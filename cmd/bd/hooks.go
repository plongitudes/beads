	...
	for hookName, hookContent := range embeddedHooks {
		...
		if _, err := os.Stat(hookPath); err == nil {
			if chain {
				...
				// cause infinite recursion. See: https://github.com/plongitudes/beads/issues/843
...
func uninstallHooks() error {
	...
	hooksDir := filepath.Join(gitDir, "hooks")
	// prepare-commit-msg kept for legacy cleanup: older bd versions installed it
	// (agent-identity trailers, since removed), so uninstall still removes stragglers.
...
func runChainedHook(hookName string, args []string) int {
	...
	// renaming an existing bd shim to .old. See: https://github.com/plongitudes/beads/issues/843
...
func runPreCommitHook() int {
	...
	// disable this and stage manually. See: https://github.com/plongitudes/beads/issues/826
...
installed bd version - upgrading bd automatically updates hook behavior.`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		case "prepare-commit-msg":
			// Removed feature (agent-identity trailers). No-op so stale shims
			// installed by older bd versions don't block commits.
			exitCode = 0
		default:
